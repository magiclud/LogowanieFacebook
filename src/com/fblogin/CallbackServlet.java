...
public class CallbackServlet extends HttpServlet {
	...
	private static String getAccessTokenFromWebContent(String webContent) {
		...
		int s = webContent.indexOf("access_token=")
				+ ("access_token=".length());
	...
	protected void doGet(HttpServletRequest request,
		...
		String code = null;
		String facebookAppId = getServletContext().getInitParameter(
				"facebookAppId");
		String facebookAppSecret = getServletContext().getInitParameter(
				"facebookAppSecret");
		...
		try {
			...
			System.out.println("String_redirectUrlbuffer " + redirectURLbuffer);
			int index = redirectURLbuffer.lastIndexOf("/");
			redirectURLbuffer.replace(index, redirectURLbuffer.length(), "")
					.append("/callback");
			...
			if (null != code) {
				...
				accessURL = "https://graph.facebook.com/oauth/access_token?client_id="
						+ facebookAppId
						+ "&redirect_uri="
						+ redirectURL
						+ "&client_secret="
